...
Useful when: an agent needs to spin up or tear down a JupyterLab session as part
of a larger workflow on the AF.

Not useful when: you need to run code _inside_ an already-running notebook —
this only manages the server's lifecycle, not its contents.
